...
| 2 | arguments | A flag, a value, or a subcommand is wrong. | Fix the command line. |
...
These come from the vendor and appear inside the message. The retry column is what the tool itself does.
...
| `-60018` | The vendor refused the call: the daily page allowance is spent. | The next day, or with another account. The pool parks the account until the next day regardless, so a same-day retry through it does not go through. |
...
- `A0202`, `A0211`, `-60018`, and `-60019` also cool the account down, so the pool stops selecting it for a while. This is why exit 5 keeps appearing after the credential is replaced: the new account has to be added, not the old one retried.
- The vendor also describes the page allowance as its top-priority tier, which it says keeps serving at lower priority, while the refusal above is explicit. Whether a lower-priority allowance survives `-60018` is not settled by the documentation; the pool treats the account as unusable for the rest of the day.
...
- When the retryable path is exhausted, the error that surfaces is the last real failure, not a substitute. `A0202` and `A0211` surface as exit 5. Every other upstream failure finishes as exit 6, with the vendor's code kept inside the message whenever it sent one.
