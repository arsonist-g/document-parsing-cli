...
| 码 | 含义 | 处置 |
|---|---|---|
| 退出码 5 | 每个账号都被拒绝，或 `parse` 发现一个账号都没配。 | 用 `docparse account test` 核实或更换凭证，缺账号时用 `docparse account add` 补一个。缺账号在 `task`、`batch`、`quota` 上是退出码 2；凭证被拒在 `task`、`batch` 上是退出码 6。重试同一次调用不会改变结果。 |
...
| `-60018` | 厂商拒绝了这次调用：当日页数额度已用完。 | 账号池会冷却该账号到次日、优先用别的账号，但没有别的可用账号时仍会用它。换一个账号立刻可用，`docparse quota` 会报告还剩多少。 |
